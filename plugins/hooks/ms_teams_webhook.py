...
#
"""
Ref: https://github.com/astronomer/cs-tutorial-msteams-callbacks/blob/main/include/hooks/ms_teams_webhook_hook.py
"""
...
class MSTeamsWebhookHook(HttpHook):
    """
    This hook allows you to post messages to MS Teams using the Incoming Webhook
    connector.

    Takes both MS Teams webhook token directly and connection that has MS Teams
    webhook token.
    If both supplied, the webhook token will be appended to the host in the
    connection.
    ...
    def get_token(self, token, http_conn_id):
        """
        Given either a manually set token or a conn_id, return the webhook_token
        to use.
        ...
        if token:
            ...
            raise AirflowException(
                'Cannot get URL: No valid MS Teams webhook URL nor conn_id '
                'supplied'
            )
